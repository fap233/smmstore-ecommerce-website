...
import { Settings, LogOut, LayoutDashboard, ShoppingCart } from "lucide-react";
...
const AdminDashboardLayout: React.FC<AdminDashboardLayoutProps> = ({ children }) => {
  ...
  const isAdmin = userRole === "ADMIN"; // Deve ser sempre true aqui, mas é bom manter a verificação

  if (!isAdmin) {
    // Redirecionar ou mostrar erro se não for admin
      ...
      <div className="hidden border-r bg-muted/40 md:block">
        <div className="flex h-full max-h-screen flex-col gap-2">
          <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
            <Link to="/dashboard/admin" className="flex items-center gap-2 font-semibold">
              <LayoutDashboard className="h-6 w-6" />
              <span className="">HydraSMM Admin</span>
            </Link>
          </div>
